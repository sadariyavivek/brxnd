...
@objc public class CKFVideoSession: CKFSession, AVCaptureFileOutputRecordingDelegate {
	...
	public var recDuration: Double = 0.0 {
		didSet {
			if let delegate = self.delegate {
				delegate.didChangeValue(session: self, value: self.recDuration, key: "duration")
			}
		}
	}
	...
	@objc func fire() {
		recDuration = movieOutput.recordedDuration.seconds
	}
